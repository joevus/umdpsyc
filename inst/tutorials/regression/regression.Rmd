...
To get regression diagnostic plots, we can simply use `plot()` on the model object. The main plots to focus on are the first two. The first one is the residual plot, which you should check to make sure there is no pattern and that there is constant variance. The second is a QQ-plot checking the normality of the residuals. 
...
```{r multmodel}
exammod2 <- lm(exam2 ~ exam1 * sleep_week, data = exam_scores)
summary(exammod2)
```
